...
* AMD SEV-SNP deployments are compatible with upstream host kernels.
* Trustee uses ECDH keys by default for KBS protocol.
* Images with many layers can be pulled without issues.
* Images with whiteouts are pulled correctly.
* The repository field of the resource URI is no longer optional.
* TDX Attestation uses configfs instead of libtdx-attest.
